...
class ClaudeAnalyzer:
    ...
    def build_system_prompt(self, script_criteria: List[Dict], equipment_type: str) -> str:
        ...
        system_prompt = f"""Ты - эксперт по оценке качества обслуживания в медицинских центрах МРТ-Лидер.

КОНТЕКСТ:
- Сеть клиник МРТ с оборудованием 1.5T и 3T
- Администраторы принимают звонки для записи на диагностику
- Есть утверждённые скрипты обслуживания для каждого типа оборудования

ЗАДАЧА:
Объективно оценить качество работы администратора по 30 критериям из скрипта {equipment_type}.

КРИТЕРИИ ОЦЕНКИ (скрипт {equipment_type}):
...
- Имя администратора: {classification.get('admin_name', 'Не указано')}
...
Проанализируй этот звонок по всем 30 критериям скрипта.
...
class QualityAnalyzer:
    ...
    def analyze_call(
        self, transcription_path: str, metadata_path: Optional[str] = None
    ) -> Dict:
        """
    ...
    def _build_system_prompt(self, script_criteria, equipment_type):
        ...
        return f"""Ты - эксперт по оценке качества обслуживания в медицинских центрах МРТ-Лидер.
...
{{
  ...
  "reasoning": "Детальное объяснение..."
    ...
    def _build_user_prompt(self, transcription, metadata):
