...
class BranchesManager:
    ...
    def get_equipment_for_address(self, address: str) -> List[str]:
        ...
        Returns:
            List[str]: Список оборудования (например, ["1.5T", "3T"])
        """
    ...
    def normalize_address(self, raw_address: str) -> str:
        ...
        for standard_address in self.addresses:
            ...
            # Например: "Ангарск, Ивана Попова" → "ул. Ивана Попова д.1Б"
    ...
    def format_for_prompt(self) -> str:
        ...
        prompt_section = f"""
ВАЖНО - АДРЕСА ФИЛИАЛОВ МРТ-ЛИДЕР (нормализуй распознанные адреса к этим вариантам):
{addresses_text}

ВАЖНО - АДМИНИСТРАТОРЫ МРТ-ЛИДЕР (исправляй имена к этим вариантам):
{admins_text}
